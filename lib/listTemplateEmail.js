...
import { isEmailHtmlDebugVerbose, logEmailHtmlDebug, logEmailMailContent } from "./listTemplateEmailDebug.js";
...
function rowsForEmailBody(section, sectionCount = 1) {
  const all = displayRows(section.headers, section.rows);
...
function buildTableRowHtml(cells, rowIdx, headers, specs, colCount) {
  return `<tr>${padRowCells(cells, colCount)
    .map((c, i) => {
      ...
      if (inner.length > 3500 || (isEmailHtmlDebugVerbose() && inner.length > 2000)) {
        console.warn(
          `[email-html][render-cell] row=${rowIdx + 1} col=${String(headers[i] ?? i)} innerLen=${inner.length} hasAnchor=${/<a[\s>]/i.test(inner)}`
        );
        if (isEmailHtmlDebugVerbose()) {
          console.warn(`[email-html][render-cell] sample:`, String(inner).slice(0, 220));
        }
      }
...
function buildDataTableHtml(headers, rows, { maxChars } = {}) {
  ...
  for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
    ...
    if (maxChars != null && charsUsed > 0 && charsUsed + rowHtml.length > maxChars) {
      console.warn(
        `[email-html][budget] HTML 예산 초과 — row ${rowIdx + 1}부터 생략 (used=${charsUsed} max=${maxChars})`
      );
      break;
    }
...
function buildSectionHtml(section, sectionCount = 1, { htmlBudgetRemaining } = {}) {
  ...
  return {
    ...
    ${empty || rows.length === 0 ? buildNoFilterMatchNoticeHtml() : `${tableBlock}${limitNotice}`}
  </div>`,
    tableChars,
  };
}
...
function buildPlainTextForSection(section, sectionCount = 1) {
  ...
  if (sectionHasNoFilterMatch(section) || rows.length === 0) {
...
export async function sendListTemplateEmail({ to, sections, note = "", attachments = [] }) {
  ...
  logEmailHtmlDebug("send", sections, content.html, { phase: "after-build" });
  if (content.html.length > GMAIL_SAFE_HTML_CHARS) {
    console.warn(
      `[email-html][send] ⚠ HTML ${content.html.length}자 — Gmail ~102KB 클립 위험 (safe=${GMAIL_SAFE_HTML_CHARS}, 본문 행 단일=${MAX_EMAIL_HTML_BODY_ROWS}, 복수섹션=${MULTI_SECTION_HTML_ROWS_EACH})`
    );
  }
  logEmailMailContent("send", content, sections, { to: toAddr });
