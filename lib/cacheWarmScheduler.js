...
export function startCacheWarmScheduler(deps, opts = {}) {
  ...
  if (!enabled) {
    console.log("[cache-warm] scheduler disabled (CACHE_WARM_SCHEDULE=0 or no token)");
    return () => {};
  }

  if (!deps.serviceKey) {
    console.warn("[cache-warm] scheduler skipped — DATA_GO_KR_SERVICE_KEY missing");
    return () => {};
  }
  ...
  const tick = async () => {
    ...
    try {
      ...
      const result = await runCacheWarm(deps, { force: true });
      const okCount = result.results.filter((r) => r.ok).length;
      console.log(
        `[cache-warm] ${started.toISOString()} refreshed ${okCount}/${result.results.length} (force=1, ok=${result.ok})`
      );
    } catch (e) {
      console.error("[cache-warm] scheduler failed:", e?.message || e);
    } finally {
      running = false;
      schedule();
    }
  ...
  const schedule = () => {
    ...
    console.log(
      `[cache-warm] next run at ${next.toLocaleString("ko-KR", { hour12: false })} (in ${Math.round(delay / 1000)}s)`
    );
