 ...
 * @param {{ codeBody: string, kind: string, expires: string|null, tier?: string, productYear?: number, updatedAt: string }} row
 */
export async function insertSeatIssued(row) {
  ...
  await c.insertOne({
    ...
    expires: row.expires,
    tier: row.tier || "cad",
    productYear: row.productYear ?? null,
    machineId: null,
    activatedAt: null,
 ...
 * @param {{ codeBody: string, kind: string, expires: string|null, tier?: string, productYear?: number, machineId: string, activatedAt: string, updatedAt: string }} row
 */
export async function claimSeatActivated(row) {
  await ensureLicenseIndexes();
  const c = await col(SEATS);
  const yearSet =
    row.productYear != null && Number.isFinite(Number(row.productYear))
      ? { productYear: Math.floor(Number(row.productYear)) }
      : {};
  ...
  if (mine) {
    ...
    return {
      ...
      expires: row.expires,
      activatedAt: row.activatedAt,
      ...(yearSet.productYear != null ? { productYear: yearSet.productYear } : {}),
    };
  }
  ...
  const doc = {
    ...
    expires: row.expires,
    tier: row.tier || "cad",
    productYear: yearSet.productYear ?? null,
    machineId: row.machineId,
    activatedAt: row.activatedAt,
    updatedAt: row.updatedAt,
    issuedAt: row.updatedAt,
