...
async function saveTabSnapshotSafe(target, payload, log) {
  const rowCount = Array.isArray(payload.rows) ? payload.rows.length : 0;
  let failReason = emptySyncFailureReason(rowCount, payload.totalCount);
...
export async function runTabSync(deps, { force = true, syncSlot = "scheduled" } = {}) {
  ...
  async function syncOneTarget(target) {
    try {
      ...
      return { tabKey: target.tabKey, ok: false, error: e?.message || "sync_failed" };
    }
  }
