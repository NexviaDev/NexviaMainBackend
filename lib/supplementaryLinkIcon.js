...
const SUP_LINK_ICON_INLINE_STYLE =
  "display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:6px;color:#5a6d9e;background:#eef2fa;border:1px solid #d8e0f0;text-decoration:none;vertical-align:middle;line-height:1;";
...
export function buildSupplementaryLinkIconHtml(href, label = SUP_LINK_ICON_DEFAULT_LABEL) {
  ...
  const safeLabel = String(label ?? SUP_LINK_ICON_DEFAULT_LABEL)
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;");
  return `<a href="${safeHref}" target="_blank" rel="noopener noreferrer" class="${NXBID_SUP_LINK_ICON_CLASS}" style="${SUP_LINK_ICON_INLINE_STYLE}" aria-label="${safeLabel}" title="${safeLabel}">${supplementaryLinkIconSvgMarkup()}</a>`;
}
