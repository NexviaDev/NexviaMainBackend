...
export function extractBizinfoListMeta(data, pageUnit) {
  const { payload: root, error } = parseUpstreamPayload(data);
  if (!root || typeof root !== "object") {
    return { totalHint: null, rowCount: 0, totalPages: 1, reqErr: error || "invalid_payload" };
  }
  if (root.reqErr) {
    return { totalHint: null, rowCount: 0, totalPages: 1, reqErr: String(root.reqErr) };
  }
  if (error) {
    return { totalHint: null, rowCount: 0, totalPages: 1, reqErr: error };
  }
