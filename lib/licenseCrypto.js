/**
 * NEXGEOM license crypto — must match license-server/server.py + CAD LicenseCode.
 * HMAC secret is derived from XOR-obfuscated constants (same as Python stub).
 *
 * Format v1 (legacy): claims[3..8] = 6-byte machine prefix; no year → treat as 2026.
 * Format v2: claims[3] = product_year − 2000; claims[4..8] = 5-byte machine prefix.
 *   Floating (legacy) = all-zero machine prefix → deterministic for same params.
 * Format v3 (floating issue): same layout as v2, but claims[4..8] = random serial
 *   (never all-zero). CAD always treats v3 as floating. Each Issue → unique code.
 */
...
/** Floating keys with random serial (unique per Issue). Always floating. */
export const FORMAT_V3 = 3;
export const LEGACY_PRODUCT_YEAR = 2026;
export const DEFAULT_PRODUCT_YEAR = 2027;
export const MIN_PRODUCT_YEAR = 2026;
export const MAX_PRODUCT_YEAR = 2099;
/** v2/v3: year byte + 5-byte machine/serial tail */
const V2_MACHINE_TAIL = 5;

/** Cryptographic serial for v3 floating — never all-zero (that would look like v2 floating). */
function randomFloatingSerial() {
  for (let attempt = 0; attempt < 8; attempt++) {
    const serial = crypto.randomBytes(V2_MACHINE_TAIL);
    if (!serial.equals(Buffer.alloc(V2_MACHINE_TAIL))) return serial;
  }
  const fallback = crypto.randomBytes(V2_MACHINE_TAIL);
  fallback[0] |= 0x01;
  return fallback;
}

/** @param {unknown} raw @param {number} [fallback] */
export function clampProductYear(raw, fallback = DEFAULT_PRODUCT_YEAR) {
  const n = Number(raw);
  if (!Number.isFinite(n)) return fallback;
  const y = Math.floor(n);
  if (y < MIN_PRODUCT_YEAR || y > MAX_PRODUCT_YEAR) {
    throw new Error(`product_year out of range (${MIN_PRODUCT_YEAR}–${MAX_PRODUCT_YEAR})`);
  }
  return y;
}

/** License covers app if license_year >= app_year (higher year unlocks older apps). */
export function licenseCoversApp(licenseYear, appYear) {
  return Number(licenseYear) >= Number(appYear);
}
...
/**
 * Issue a floating (online) key. Format v3: product year + random serial → unique code each Issue.
 * @param {"perpetual"|"term"} kind
 * @param {Date|null} expires
 * @param {number} [productYear]
 */
export function issueFloating(kind, expires, productYear = DEFAULT_PRODUCT_YEAR) {
  const year = clampProductYear(productYear);
  ...
  claims[0] = (FORMAT_V3 << 4) | (typeBits & 0x03);
  claims[1] = (dayIndex >> 8) & 0xff;
  claims[2] = dayIndex & 0xff;
  claims[3] = (year - 2000) & 0xff;
  randomFloatingSerial().copy(claims, 4);
...
export function parseCode(code) {
  ...
  let floating;
  if (ver === FORMAT_V1) {
    productYear = LEGACY_PRODUCT_YEAR;
    const machinePrefix = claims9.subarray(3, 9);
    floating = machinePrefix.equals(Buffer.alloc(6));
  } else if (ver === FORMAT_V2) {
    productYear = 2000 + claims9[3];
    if (productYear < MIN_PRODUCT_YEAR || productYear > MAX_PRODUCT_YEAR) {
      throw new Error("invalid product_year in key");
    }
    const machineTail = claims9.subarray(4, 4 + V2_MACHINE_TAIL);
    floating = machineTail.equals(Buffer.alloc(V2_MACHINE_TAIL));
  } else if (ver === FORMAT_V3) {
    productYear = 2000 + claims9[3];
    if (productYear < MIN_PRODUCT_YEAR || productYear > MAX_PRODUCT_YEAR) {
      throw new Error("invalid product_year in key");
    }
    // v3 is always floating (serial in claims[4..8] is not a machine bind).
    floating = true;
  } else {
