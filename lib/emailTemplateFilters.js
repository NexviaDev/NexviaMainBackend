...
function pickFirst(row, keys) {
  for (const k of keys) {
    const v = normalizeSnapshotFieldText(row?.[k]);
    if (v) return v;
  }
  return "";
}

/** MongoDB/API 스냅샷 — CDATA·태그 제거 (필터·메일 표시 동일 기준) */
export function normalizeSnapshotFieldText(raw) {
  return String(raw ?? "")
    .replace(/<!\[CDATA\[|\]\]>/g, "")
    .replace(/<[^>]*>/g, " ")
    .replace(/&lt;/gi, "<")
...
function bidRowCellFilterText(row, col, naraBiz, rowIndex, rowTotal) {
  switch (col) {
    ...
    case "bidNtceNm":
      return normalizeSnapshotFieldText(row.bidNtceNm);
    case "bidNtceNo":
      return noticeNoLine(row);
    case "industry":
    ...
    default: {
      const direct = normalizeSnapshotFieldText(row?.[col]);
...
function parseRelativeDaysFilter(raw) {
  const v = String(raw).trim();
  const prefixed = v.match(/^d:(\d{1,4})$/);
  if (prefixed) {
    const days = Number(prefixed[1]);
    if (days > 0 && days <= MAX_RELATIVE_DAYS) return { days };
    return null;
  }
  if (/^\d{1,4}$/.test(v)) {
    const days = Number(v);
...
function resolveDateRangeFilter(raw, now = new Date()) {
  const rel = parseRelativeDaysFilter(raw);
  if (rel) {
    const to = formatYmd(now);
    const fromD = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    fromD.setDate(fromD.getDate() - rel.days);
    return { from: formatYmd(fromD), to };
  }
  return parseDateRangeFilter(raw);
}
...
function supplementaryCellText(row, col) {
  ...
  return v == null ? "" : normalizeSnapshotFieldText(v);
}
  ...
  return rows.filter((row, idx) => {
    for (const [col, raw] of entries) {
      if (BID_DATE_COLS.has(col)) {
        const range = resolveDateRangeFilter(raw);
        if (range) {
