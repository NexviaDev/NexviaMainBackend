...
export async function upsertTabSnapshot(tabKey, payload) {
  ...
  const write = await db.collection(TAB_SNAPSHOTS_COLLECTION).updateOne(
    { tabKey },
    { $set: doc },
    { upsert: true }
  );
  ...
  if (doc.rowCount > 0) {
    const verify = await db.collection(TAB_SNAPSHOTS_COLLECTION).findOne(
      { tabKey },
      { projection: { rowCount: 1, rows: { $slice: 1 } } }
    );
    const stored = Number(verify?.rowCount) || 0;
    if (stored !== doc.rowCount) {
      throw new Error(`mongodb_verify_failed:${tabKey}:wrote_${doc.rowCount}_stored_${stored}`);
    }
  }
