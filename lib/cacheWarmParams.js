...
/**
 * tab-sync·cache-warm upstream 페이지 크기 (화면 UI 50/20 과 별개)
 * 100 — API 안정·응답 크기 균형 (50 대비 호출 약 50% 감소)
 */
export const UPSTREAM_BID_NUM_OF_ROWS = 100;
export const UPSTREAM_PRESPEC_NUM_OF_ROWS = 100;
export const UPSTREAM_BIZINFO_PAGE_UNIT = 100;
export const UPSTREAM_EVENTS_PAGE_UNIT = 100;
export const UPSTREAM_BIZINFO_SEARCH_CNT = 100;
...
export function buildBidWarmQueries() {
  ...
  const base = {
    ...
    inqryEndDt,
    pageNo: "1",
    numOfRows: String(UPSTREAM_BID_NUM_OF_ROWS),
    type: "json",
  };
  return [
    {
...
export function buildPrespecWarmQuery() {
  ...
  return {
    ...
    query: {
      ...
      pageNo: "1",
      numOfRows: String(UPSTREAM_PRESPEC_NUM_OF_ROWS),
      type: "json",
    },
  };
}
...
export function buildBizinfoWarmQuery() {
  return {
    ...
    query: {
      dataType: "json",
      pageUnit: String(UPSTREAM_BIZINFO_PAGE_UNIT),
      pageIndex: "1",
      searchCnt: String(UPSTREAM_BIZINFO_SEARCH_CNT),
    },
...
export function buildEventsWarmQuery() {
  return {
    ...
    query: {
      dataType: "json",
      pageUnit: String(UPSTREAM_EVENTS_PAGE_UNIT),
      pageIndex: "1",
    },
